...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  const content = createMemo(() => normalizeLineSpaces(props.part.text.trim()))
  return (
    <Show when={content()}>
...
function ShimmerText(props: { badge: string; text: string }) {
  ...
  return (
    <box flexDirection="column">
      <box backgroundColor={theme.backgroundElement} flexShrink={0} width={badgeText().length}>
...
function Write(props: ToolProps<typeof WriteTool>) {
  ...
  const hasContent = createMemo(() => code().length > 0)
