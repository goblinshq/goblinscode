...
export namespace Installation {
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        cmd = $`curl -fsSL https://raw.githubusercontent.com/goblinshq/goblinscode/main/install | bash`.env({
          ...process.env,
          VERSION: target,
        })
        break
  ...
  export async function latest(installMethod?: Method) {
    ...
    return fetch("https://api.github.com/repos/goblinshq/goblinscode/releases/tags/goblins-latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })
